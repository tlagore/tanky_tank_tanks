...
void render_splash(UINT32 *base32)
{
	...
	int i, j;
	int x = SPLASH_SQUARE_START_X, 
		y = SPLASH_SQUARE_START_Y,
		width = SPLASH_SQUARE_WIDTH, 
		height = SPLASH_SQUARE_HEIGHT;
	
	
	plot_square(base32, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, FILL);
	clear_area(base32, 54, 585, 20, 60);
	/*plot_square(base32, 0, 50, 639, 349, FILL);*/
	...
	render_string ((UINT8*)base32, EXIT_X, EXIT_Y, "quit", TRUE);
...
void render_splash_tanks(UINT32 *base32)
{
	...
	plot_bitmap_32(base32, TWOP_TANK1_X, TWOP_TANK1_Y,
		tank_bitmap, TANK_HEIGHT);
	
	plot_bitmap_16(base16, TWOP_TANK1_X + cannon_x_offset[PLAYER_ONE],
		TWOP_TANK1_Y - CANNON_Y_OFFSET, cannon_bitmap[PLAYER_ONE][0],
		CANNON_HEIGHT);

	plot_bitmap_32(base32, TWOP_TANK2_X, TWOP_TANK2_Y,
		tank_bitmap, TANK_HEIGHT);
	
	plot_bitmap_16(base16, TWOP_TANK2_X + cannon_x_offset[PLAYER_TWO],
		TWOP_TANK2_Y - CANNON_Y_OFFSET, cannon_bitmap[PLAYER_TWO][0],
		CANNON_HEIGHT);
}
...
void render_how(UINT32 *base32)
{
	...
	clear_area(base32, SPLASH_SQUARE_START_X, 570, 
		SPLASH_SQUARE_START_Y, 272); 

	...
	const struct Tank tanks[], int * collision)
{
	...
	missile->x += (missile->deltaX >> 4);
	missile->y += (missile->deltaY >> 3);
	
	/* check for bounds collision */
	if (terrainPos >= -1 && terrainPos <= 39)
		*(collision) = check_terrain_collision(missile->x, 
					missile->y + missile->height, terrain, 
					terrainPos);
		
	
	/* check for collision with tanks */
	for (i = 0; i < NUM_TANKS && !(*collision); i++)
		if (check_collision(missile->x,
				...
				tanks[i].height))
				{
					*(collision) = TANK_COLLISON_OFFSET + i;
					event_tank_hit(&tanks[i], collision);
				}

	/* x boundary checking and staging area collision detection */
	if (!(*collision))
		if (missile->x < 0 || (int)(missile->x + missile->width) >= SCREEN_WIDTH 
			|| (int)(missile->y + missile->height) > LOWEST_TERRAIN_POINT)

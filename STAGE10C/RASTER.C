...
#define SDISPLAY_VBASE_HIGH 0xFFFF8201
#define SDISPLAY_VBASE_MED 0xFFFF8203


volatile UINT8 * const sdisplay_vbase_high = (UINT8*)SDISPLAY_VBASE_HIGH;
volatile UINT8 * const sdisplay_vbase_med = (UINT8*)SDISPLAY_VBASE_MED;
...
void set_screen(volatile const UINT8* sdisplay_vbase_high, UINT16 *base);
...
UINT16 *get_video_base()
{
	...
	baseAddress = *sdisplay_vbase_high;
	baseAddress <<= 8;
	baseAddress |= *sdisplay_vbase_med;
	baseAddress <<= 8;
	
	basePointer = (UINT16*) baseAddress;
...
void set_video_base(UINT16 * base16)
{
	UINT32 old_ssp = Super(0);
	set_screen(sdisplay_vbase_high, base16);
	Super(old_ssp);
}
			...
			const UINT8 * bitmap, UINT16 height)
{
	...
	if (y + height > 0 && x > 0 && x < SCREEN_WIDTH - BYTE)
	{
		if (y < 0)
		{
			height += y;

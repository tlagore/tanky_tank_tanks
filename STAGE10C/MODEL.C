...
Purpose: randomizes the terrain over an arc.

The first and last terrain piece are kept flat to avoid
	having clipping issues.  The function works by iterating
	over the array of terrain objects, adding a hard offset 
	up to the halfpoint, at which point the hard offset is
	subtracted until the end.  Each piece is randomly chosen
	to add or subtract an offset to the current y and is set at 
	that location.
...
/*
Name: reset_model
Purpose: Resets the model to the games starting values to initialize,
	or restart the game.
*/
